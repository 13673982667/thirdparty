...
class logging_duckchat {
            ...
            if($result['status'] > 0) {
                ...
                if(empty($_GET['handlekey']) || !empty($_GET['lssubmit'])) {
                    if(defined('IN_MOBILE')) {
                        $this->jumpToSite($_G['sid'], 'location_login_succeed_mobile', $location, array('username' => $result['ucresult']['username']), '');
                    } else {
                        if(!empty($_GET['lssubmit'])) {
                            if(!$ucsynlogin) {
                                $extra['location'] = true;
                            }
                            $this->jumpToSite( $_G['sid'], $loginmessage, $location, $param, $extra);
                        } else {
                            ...
                            $this->jumpToSite($_G['sid'],"location_login_succeed", $location, $param, $extra);
                        }
                    }
                } else {
                    $this->jumpToSite($_G['sid'],$loginmessage, $location, $param, $extra);
    ...
    function jumpToSite($pressionId, $loginmessage, $location, $param, $extra) {
        ...
        if($duckchatCookie == "duckchat") {
            ...
            showmessage('success, jump to site', '', array('type' => 1), $extra);
            die();
        }
...
class register_duckchat {
    ...
    function on_register() {
        ...
        if($_G['uid']) {
            ...
            $this->jumpToSite($_G['sid'], "login_succeed", $url_forward,  array('username' => $_G['member']['username'], 'usergroup' => $_G['group']['grouptitle'], 'uid' => $_G['uid']), array('extrajs' => $ucsynlogin));
        ...
        error_log("shaoye ------------------------shaoye ------------------------shaoye ------------------------");
        ...
        if(!submitcheck('regsubmit', 0, $seccodecheck, $secqaacheck)) {
            ...
            error_log("shaoye ---------33333---------------shaoye ------------------------shaoye ------------------------");
            ...
            error_log("shaoye ---------55555---------------shaoye ------------------------shaoye ------------------------");
            ...
            error_log("shaoye ---------6666666---------------shaoye ------------------------shaoye ------------------------");
            ...
            error_log("shaoye ---------777777---------------shaoye ------------------------shaoye ------------------------");
            ...
            $this->jumpToSite($_G['sid'], $message, $url_forward, $param, $extra);
        }
    }
    ...
    function jumpToSite($pressionId, $loginmessage, $location, $param, $extra) {
        ...
        if($duckchatCookie == "duckchat") {
            ...
            showmessage('success, jump to site', '', [], $extra);
            die();
        }
        showmessage($loginmessage, $location, $param, $extra);
    }
