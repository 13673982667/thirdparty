...
class logging_duckchat {

    function logging_duckchat() {
        ...
        if(!submitcheck('loginsubmit', 1, $seccodestatus)) {
            ...
            if($result['status'] == -1) {
                if(!$this->setting['fastactivation']) {
                    ...
                    showmessage('location_activation', 'member.php?mod='.$this->setting['regname'].'&action=activation&fromWhere=duckchat&auth='.rawurlencode($auth).'&referer='.rawurlencode(dreferer()), array(), array('location' => true));
                } else {
            ...
            if($result['status'] > 0) {
                ...
                if($_G['member']['adminid'] != 1) {
                    if($this->setting['accountguard']['loginoutofdate'] && $_G['member']['lastvisit'] && TIMESTAMP - $_G['member']['lastvisit'] > 90 * 86400) {
                        ...
                        showmessage('location_login_outofdate', 'home.php?mod=spacecp&from=duckchat&ac=profile&op=password&resend=1', array('type' => 1), array('showdialog' => true, 'striptags' => false, 'locationtime' => true));
                    }
                ...
                if(empty($_GET['handlekey']) || !empty($_GET['lssubmit'])) {
                    if(defined('IN_MOBILE')) {
                       $this->jumpToSite($_G, $loginmessage, $location, $param, '');
                    } else {
                        if(!empty($_GET['lssubmit'])) {
                            if(!$ucsynlogin) {
                                $extra['location'] = true;
                            }
                            $this->jumpToSite($_G, $loginmessage, $location, $param, $extra);
                        } else {
                            ...
                            $this->jumpToSite($_G, "location_login_succeed", $location, $param, $extra);
                        }
                    }
                } else {
                    $this->jumpToSite($_G, $loginmessage, $location, $param, $extra);
                ...
                if($_G['member_loginperm'] > 1) {
                    $this->jumpToSite($_G, $fmsg, "", array('loginperm' => $_G['member_loginperm'] - 1));
...
class register_duckchat {

    var $showregisterform = 1;

    function register_duckchat() {
        global $_G;
    ...
    function on_register() {
        ...
        if($_G['uid']) {
            ...
            $this->jumpToSite($_G, "login_succeed", $url_forward, array('username' => $_G['member']['username'], 'usergroup' => $_G['group']['grouptitle'], 'uid' => $_G['uid']), array('extrajs' => $ucsynlogin));
        ...
        if(!submitcheck('regsubmit', 0, $seccodecheck, $secqaacheck)) {
            ...
            $this->jumpToSite($_G, $message, $url_forward, $param, $extra);
        }
    }
